...
class BulletinService(BaseService):
    ...
    def get_bulletin_list(self, data={}):
        ...
        sql = "SELECT bulletins.*, users.account as setter_user FROM bulletins, users WHERE bulletins.group_id = %s and bulletins.setter_user_id = users.id order by bulletins.id limit %s, %s"
