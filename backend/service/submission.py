...
class SubmissionService(BaseService):
    ...
    def get_submission(self, data):
        ...
        if res_cnt == 0:
            return ('No Submission ID', None)
        res = res[0]
        res['testdata'], res_cnt = yield from self.db.execute('SELECT m.*, v.* FROM map_submission_testdata as m, map_verdict_string as v WHERE submission_id=%s AND v.id=m.veridct;', (data['id'],))
