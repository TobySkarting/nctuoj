#!/bin/sh
BUILD='build'
sudo apt-get isntall libreadline6 libreadline6-dev 
mkdir $BUILD
cd $BUILD
...
cd src && make dependencies && make x64.release library=shared soname_version=1.0 console=readline snapshot=off werror=no
